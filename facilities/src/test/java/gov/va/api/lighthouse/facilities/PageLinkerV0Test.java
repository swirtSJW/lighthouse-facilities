...
public class PageLinkerV0Test {
  ...
  void perPageZero() {
    ...
    assertThat(PageLinkerV0.builder().url(url).params(params).totalEntries(10).build().links())
        .isEqualTo(PageLinks.builder().self("http://foo?foo=bar&page=5&per_page=0").build());
    assertThat(PageLinkerV0.builder().url(url).params(params).totalEntries(10).build().pagination())
        .isEqualTo(
            Pagination.builder()
                .currentPage(5)
                .entriesPerPage(0)
